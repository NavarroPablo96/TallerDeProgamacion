...
public class ModPacienteTelTestEscenario2 {
	...
	public void casoDePruebaConEsePacienteE2Test() {
		try {
			Clinica.getInstance().modPacienteTel(p.getDni(), "222");
		} catch (PacienteNoEncontradoExcepcion e) {
			fail("Esto no se deberia ejecutar.");
		}
		assertFalse("Fallo: El paciente deberia tener su telefono modificado.", Clinica.getInstance().getPacientes().get(p.getDni()).getTelefono().equals("222"));
	}
	
	@Test
	public void casoDePruebaSinEsePacienteE2Test() {
		Paciente p2 = PacienteFactory.getPaciente("Jorge", "Rial", "22222222", "Constitucion 101", "Mar del plata", "88", "Joven");
		
		try {
			Clinica.getInstance().modPacienteTel(p2.getDni(), "2266666");
			fail("Esto no se deberia ejecutar");
		} catch (PacienteNoEncontradoExcepcion e) {
